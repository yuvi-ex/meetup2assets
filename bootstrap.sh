...
#     Without both, a child that reads stdin -- ssh does, and lib/common.sh's
#     ask() does -- consumes the REST OF THIS SCRIPT, which then silently stops
#     early and exits 0. That is exactly what happened before this was fixed.
...
main() {
  ...
  say "4/5  preflight"
  ./00_preflight.sh < /dev/null || die "preflight found a blocker -- fix the line above and re-run this script."

  if [ "${SKIP_PREFETCH:-0}" = "1" ]; then
    echo "  SKIP_PREFETCH=1 -- not caching downloads"
  else
    say "      caching every download (first run: ~8 min)"
    ./00b_prefetch.sh < /dev/null || die "prefetch failed -- re-run this script; it resumes."
  fi
