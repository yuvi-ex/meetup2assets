-- Breakdown by Customer City. Uniform NAME/rank shape so one code path serves every cut.
SELECT g."CUSTOMER_CITY" AS NAME,
       COUNT(*) AS LINES_N,
  ...
  FROM "RETAIL"."ORDERS_ENRICHED" g
   WHERE ({f0!s} = '*' OR INSTR({f0!s}, '|' || TO_CHAR(g."CUSTOMER_SEGMENT") || '|') > 0)
     ...
     AND ({f3!s} = '*' OR INSTR({f3!s}, '|' || TO_CHAR(g."FAV_CATEGORY") || '|') > 0) GROUP BY 1 ORDER BY REVENUE_SUM DESC LIMIT 60
