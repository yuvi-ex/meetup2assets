-- Row-level worklist, bounded so Exasol never streams the table.
SELECT g."CUSTOMER_CITY" AS CUSTOMER_CITY,
       g."CUSTOMER_SEGMENT" AS CUSTOMER_SEGMENT,
       COUNT(*) AS LINES_N,
  ...
  FROM "RETAIL"."ORDERS_ENRICHED" g
   WHERE ({f0!s} = '*' OR INSTR({f0!s}, '|' || TO_CHAR(g."CUSTOMER_SEGMENT") || '|') > 0)
     AND ({f1!s} = '*' OR INSTR({f1!s}, '|' || TO_CHAR(g."AGE_BAND") || '|') > 0)
     AND ({f2!s} = '*' OR INSTR({f2!s}, '|' || TO_CHAR(g."CUSTOMER_CITY") || '|') > 0)
     AND ({f3!s} = '*' OR INSTR({f3!s}, '|' || TO_CHAR(g."FAV_CATEGORY") || '|') > 0) GROUP BY 1, 2 ORDER BY REVENUE_SUM DESC LIMIT 200
