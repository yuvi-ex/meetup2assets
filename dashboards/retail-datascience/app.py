...
Data Scientist would actually do about each number. Two rules to keep while you do:
...
ADDITIVE = ['REVENUE_SUM', 'QUANTITY_SUM', 'NET_REVENUE_SUM', 'GROSS_MARGIN_SUM']
KEYS = []
FRAMES = ['trend.sql', 'cut_a.sql', 'cut_b.sql', 'detail.sql']
DETAIL_COLS = ['CUSTOMER_CITY', 'CUSTOMER_SEGMENT', 'LINES_N', 'REVENUE_SUM', 'QUANTITY_SUM', 'NET_REVENUE_SUM']
MAIN = "REVENUE_SUM"
MAIN_ALT = "REVENUE_AVG"
MAIN_AVG = "REVENUE_AVG"
MAIN_LABEL = "Booked Revenue"
CUT_A, CUT_B = "Customer City", "Segment"
...
NEGATIVES = [
    'No event stream. One row per order line, no sessions, clicks or basket sequence.',
    'No time dimension on the customer attributes — they are a current-state snapshot, so nothing can be lagged and no leakage check is possible.',
    'No holdout period worth the name: the fact table spans 2025-01-01 to 2025-10-27 with a flat monthly shape.',
    'No labels for anything a model would predict — churn, propensity and fraud are all absent.',
]
REFUSED = [
    ...
    'Do customer attributes predict revenue? No. Controlling for the product band, age band falls to a 0.0-0.2% spread, tier to 0.4-0.9%, favourite category to 1.1-1.4% and support tickets to 1.2-3.1%. Every raw effect of 84-109% was the product assignment showing through.',
    'Is satisfaction related to spend? No. satisfaction_score sits at 3.97-4.02 across all three segments and does not move with revenue.',
    'Can loyalty points be modelled as engagement? Not usefully — the points band spread is 12.6% raw and collapses under the same control.',
]
...
ASK_JOINS = [
    ...
    'CONFOUND — this is the whole point of this board. Every raw customer '
    'attribute effect is the product band showing through. Controlling for it, '
    'age band falls to a 0.0-0.2% spread, loyalty tier to 0.4-0.9%, favourite '
    'category to 1.1-1.4% and support tickets to 1.2-3.1%. Any raw effect of '
    '80-110% you compute is the product assignment, so report it as ruled out '
    'rather than as a finding.',
    'SATISFACTION sits at 3.97-4.02 across all three segments and does not move '
    'with revenue. There are only 10 customer cities. No event stream, no '
    'sessions, no basket sequence, no labels for churn, propensity or fraud, and '
    'no usable holdout period — so no model can be built or validated here.',
]
...
def build_insights(kpi, *frames):
    ...
    rev = _f(kpi.get("REVENUE_SUM"))
    out.append({
        "tone": "info",
        "text": (f"{int(lines):,} lines, {_val(rev)}, "
                 f"{int(_f(kpi.get('CUSTOMERS_N'))):,} customers — feature audit"),
        "meaning": ("This board tests whether the MongoDB customer attributes predict "
                    "anything in the Exasol order facts. It reports what survives a "
                    "control, not what looks big in a raw cut."),
        "action": "Read the cards below before modelling on any customer feature.",
    })
    sp_city = _spread(cut_a, "AVG_ORDER")
    if sp_city and sp_city["lo"]:
        ratio = sp_city["hi"] / sp_city["lo"]
        out.append({
            "tone": "bad" if ratio >= 2 else "warn",
            "text": (f"Customer city appears to swing line value {ratio:.1f}x — "
                     f"it is an artifact"),
            "meaning": ("The cities split into two non-overlapping bands because each "
                        "product appears in exactly one band and quantity takes only "
                        "{2,4} in one and {1,3} in the other. City is a proxy for the "
                        "product a row was assigned, not a market."),
            "action": "Never use customer city as a feature against this fact table.",
        })
    sp_seg = _spread(cut_b, "AVG_ORDER")
    if sp_seg and sp_seg["mean"]:
        flat = sp_seg["relative"] < FLAT_PCT
        out.append({
            "tone": "good" if flat else "warn",
            "text": (f"Customer segment is {'ruled out' if flat else 'uneven'} on line "
                     f"value ({sp_seg['relative']:.1f}% spread)"),
            "meaning": (f"{_val(sp_seg['lo'])} to {_val(sp_seg['hi'])} across "
                        f"{sp_seg['groups']} segments. Segment and loyalty tier are the "
                        f"same column — one-to-one with no exceptions — so this rules "
                        f"out both at once."),
            "action": ("Segment carries no signal; drop it from any revenue model."
                       if flat else f"Check {sp_seg['hi_name']} for a real difference."),
        })
    rating = pct("RATING_AVG")
    disc = pct("DISCOUNT_AVG")
    if rating and disc:
        out.append({
            "tone": "bad",
            "text": f"RATING is a restatement of DISCOUNT_PCT, not an opinion",
            "meaning": (f"Discount maps to rating one-to-one across all rows: 0%->1 star, "
                        f"5%->2, 10%->3, 15%->4, 20%->5. Current scope averages "
                        f"{rating:.2f} stars at {disc:.1f}% discount."),
            "action": "Any 'discounts raise satisfaction' finding from this is circular.",
        })
    out.append({
        "tone": "bad",
        "text": "The customer dimension's lifetime_value contradicts the orders",
        "meaning": ("retail_customers.json states a 6x LTV spread across segments and "
                    "~19 orders per customer. The fact table shows a 0.3% revenue spread "
                    "and exactly 10 orders for every customer. The two files were "
                    "generated independently and share only customer IDs."),
        "action": "Derive value from RETAIL.ORDERS; treat stated LTV as unusable.",
    })
