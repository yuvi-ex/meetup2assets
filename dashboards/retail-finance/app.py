...
Finance Manager would actually do about each number. Two rules to keep while you do:
...
ADDITIVE = ['REVENUE_SUM', 'NET_REVENUE_SUM', 'LOST_REVENUE_SUM', 'GROSS_MARGIN_SUM']
KEYS = []
FRAMES = ['trend.sql', 'cut_a.sql', 'cut_b.sql', 'detail.sql']
DETAIL_COLS = ['CATEGORY', 'ORDER_STATUS', 'LINES_N', 'REVENUE_SUM', 'NET_REVENUE_SUM', 'LOST_REVENUE_SUM']
MAIN = "REVENUE_SUM"
MAIN_ALT = "REVENUE_AVG"
MAIN_AVG = "REVENUE_AVG"
MAIN_LABEL = "Booked Revenue"
CUT_A, CUT_B = "Category", "Order Status"
...
NEGATIVES = [
    'No cost drivers. COST is a fixed 55% of revenue on every line, so there is no input-cost, freight or labour component to analyse.',
    'No cancellation or return reason, and no status timestamp — the leak can be sized, never explained or aged.',
    'No customer credit, invoice or payment-timing data. PAYMENT_METHOD is the method, not a receivable.',
    'No budget or forecast to vary against. Nothing here says what revenue should have been.',
]
REFUSED = [
    ...
    'Which category is most profitable? None is. Gross margin is 45.0% in all seven, a spread under 0.1pp, so mix cannot move profitability and no category is favoured.',
    'Does payment method affect realisation? No. The five methods sit within noise of each other on both realised share and return rate.',
]
...
ASK_JOINS = [
    ...
    'COST is a fixed 55% of REVENUE, the exact mirror of GROSS_MARGIN, so there '
    'are no input-cost, freight or labour drivers to analyse and no cost question '
    'has an answer beyond that identity.',
    'No cancellation or return reason code and no status timestamp anywhere. The '
    '4,880,883 leak can be SIZED by status, category or store, but never '
    'explained, attributed or aged. PAYMENT_METHOD is the method, not a '
    'receivable, and there is no budget or forecast to vary against.',
]
...
def build_insights(kpi, *frames):
    ...
    pending = booked - net - lost
    out.append({
        "tone": "info",
        "text": (f"{_val(booked)} booked, {_val(net)} realised, "
                 f"{_val(lost)} lost"),
        "meaning": (f"{int(lines):,} order lines. Booked is every line regardless of "
                    f"outcome; realised is Delivered plus Shipped; lost is Cancelled "
                    f"plus Returned. {_val(pending)} is still Processing — neither yet."),
        "action": "Report realised, not booked, in anything that reaches the P&L.",
    })
    if booked:
        lost_pct = lost / booked * 100
        out.append({
            "tone": "bad" if lost_pct >= 25 else "warn" if lost_pct >= 12 else "good",
            "text": f"{lost_pct:.1f}% of booked revenue never converts ({_val(lost)})",
            "meaning": ("Cancellations and returns together. There is no cancellation "
                        "reason, return reason or timestamp in the source, so this "
                        "sizes the leak without explaining it."),
            "action": "Get reason codes into the feed before targeting this.",
        })
        out.append({
            "tone": "warn" if pending / booked > 0.10 else "info",
            "text": f"{_val(pending)} sits in Processing with no outcome yet",
            "meaning": (f"{pending / booked * 100:.1f}% of booked revenue. These lines "
                        f"also carry no delivery time, so they cannot be aged."),
            "action": "Treat as unrecognised revenue until the status resolves.",
        })
    sp = _spread(cut_a, MAIN)
    if sp and sp["mean"]:
        share = sp["hi"] / booked * 100 if booked else 0
        out.append({
            "tone": "warn" if share >= 30 else "info",
            "text": (f"{sp['hi_name']} carries {share:.0f}% of booked revenue "
                     f"({_val(sp['hi'])})"),
            "meaning": (f"Across {sp['groups']} categories, {_val(sp['hi'])} against "
                        f"{_val(sp['lo'])} in {sp['lo_name']} — a {sp['relative']:.0f}% "
                        f"spread of the mean."),
            "action": f"Concentration risk sits with {sp['hi_name']}.",
        })
