-- Breakdown by Order Status. Uniform NAME/rank shape so one code path serves every cut.
SELECT g."ORDER_STATUS" AS NAME,
       COUNT(*) AS LINES_N,
  ...
  FROM "RETAIL"."ORDERS_ENRICHED" g
   WHERE ({f0!s} = '*' OR INSTR({f0!s}, '|' || TO_CHAR(g."ORDER_STATUS") || '|') > 0)
     ...
     AND ({f3!s} = '*' OR INSTR({f3!s}, '|' || TO_CHAR(g."PAYMENT_METHOD") || '|') > 0) GROUP BY 1 ORDER BY REVENUE_SUM DESC LIMIT 60
