...
SELECT g."STORE_NAME" AS NAME,
  ...
  FROM "RETAIL"."ORDERS_ENRICHED" g
   WHERE ({f0!s} = '*' OR INSTR({f0!s}, '|' || TO_CHAR(g."STORE_NAME") || '|') > 0)
     AND ({f1!s} = '*' OR INSTR({f1!s}, '|' || TO_CHAR(g."CHANNEL") || '|') > 0)
     AND ({f2!s} = '*' OR INSTR({f2!s}, '|' || TO_CHAR(g."CATEGORY") || '|') > 0)
     AND ({f3!s} = '*' OR INSTR({f3!s}, '|' || TO_CHAR(g."ORDER_STATUS") || '|') > 0) GROUP BY 1 ORDER BY NET_REVENUE_SUM DESC LIMIT 60
