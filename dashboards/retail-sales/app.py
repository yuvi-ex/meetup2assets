...
Sales Manager would actually do about each number. Two rules to keep while you do:
...
ADDITIVE = ['NET_REVENUE_SUM', 'REVENUE_SUM', 'LOST_REVENUE_SUM', 'QUANTITY_SUM']
KEYS = []
FRAMES = ['trend.sql', 'cut_a.sql', 'cut_b.sql', 'detail.sql']
DETAIL_COLS = ['STORE_NAME', 'CHANNEL', 'LINES_N', 'NET_REVENUE_SUM', 'REVENUE_SUM', 'LOST_REVENUE_SUM']
MAIN = "NET_REVENUE_SUM"
MAIN_ALT = "NET_REVENUE_AVG"
MAIN_AVG = "NET_REVENUE_AVG"
MAIN_LABEL = "Net Revenue"
CUT_A, CUT_B = "Store", "Channel"
...
NEGATIVES = [
    'No sales rep. SELLER_ID identifies a marketplace seller, not a person with a quota.',
    'No pipeline or opportunity data — only orders that already exist.',
    'No quota, target or territory definition, so nothing can be reported as attainment.',
    'No visit, call or campaign activity to attribute a win to.',
]
REFUSED = [
    ...
    'Which channel should get more spend? None on this evidence — Mobile App, Store and Web sit within a few percent of each other on realised revenue per line.',
    'Is any store a genuine outperformer? Not per line. Store revenue differences track line counts, so they are coverage facts rather than performance ones.',
]
...
ASK_JOINS = [
    ...
    'SELLER_ID is a marketplace seller identifier, NOT a salesperson with a '
    'quota. There is no rep, pipeline, opportunity, quota, target, territory or '
    'campaign anywhere in this data, so nothing can be reported as attainment.',
    'The six stores carry 416-417 lines each. Their revenue differences therefore '
    'track line counts, which makes them coverage facts and not performance ones '
    '— rank stores only on a per-line or per-customer basis, and say when the '
    'spread is noise.',
]
...
def build_insights(kpi, *frames):
    ...
    out.append({
        "tone": "info",
        "text": (f"{_val(net)} realised from {int(lines):,} lines across "
                 f"{int(_f(kpi.get('CUSTOMERS_N'))):,} customers"),
        "meaning": (f"{_val(booked)} was booked; {_val(lost)} was lost to "
                    f"cancellations and returns. Average line {_val(kpi.get(MAIN_AVG))}."),
        "action": "Quota should track realised, not booked.",
    })
    sp = _spread(cut_a, MAIN)
    if sp and sp["lo"]:
        ratio = sp["hi"] / sp["lo"]
        out.append({
            "tone": "warn" if ratio >= 1.5 else "good",
            "text": (f"{sp['hi_name']} leads on realised revenue, {sp['lo_name']} "
                     f"trails ({_val(sp['hi'])} against {_val(sp['lo'])})"),
            "meaning": (f"{ratio:.1f}x across {sp['groups']} stores. Check the row count "
                        f"alongside it — a store can lead on revenue purely by holding "
                        f"more lines, which is a coverage fact, not a performance one."),
            "action": f"Compare {sp['hi_name']} and {sp['lo_name']} per line before ranking anyone.",
        })
    spv = _spread(cut_a, "AVG_ORDER")
    if spv and spv["mean"]:
        flat = spv["relative"] < FLAT_PCT
        out.append({
            "tone": "good" if flat else "warn",
            "text": (f"Average line value is {'even' if flat else 'uneven'} across stores "
                     f"({spv['relative']:.0f}% spread)"),
            "meaning": (f"{_val(spv['lo'])} to {_val(spv['hi'])}. "
                        + ("So store revenue differences are volume, not basket — "
                           "the lever is traffic, not upsell."
                           if flat else
                           f"{spv['hi_name']} sells a materially bigger line than "
                           f"{spv['lo_name']}.")),
            "action": ("Chase footfall and line count, not average order value."
                       if flat else f"Study what {spv['hi_name']} does differently."),
        })
    spc = _spread(cut_b, MAIN)
    if spc and spc["mean"]:
        flat = spc["relative"] < FLAT_PCT
        out.append({
            "tone": "good" if flat else "warn",
            "text": (f"Channel is {'ruled out' if flat else 'uneven'}: "
                     f"{spc['relative']:.0f}% spread across {spc['groups']} channels"),
            "meaning": (f"{spc['hi_name']} {_val(spc['hi'])} against {spc['lo_name']} "
                        f"{_val(spc['lo'])}. "
                        + ("Below the point where a difference is real at these volumes."
                           if flat else "Wide enough to attribute.")),
            "action": ("Do not shift spend between channels on this evidence."
                       if flat else f"Start with {spc['hi_name']}."),
        })
