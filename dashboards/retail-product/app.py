...
Product Manager would actually do about each number. Two rules to keep while you do:
...
FILTER_SPEC = [
    ...
    ('f1', 'SUBCATEGORY', 'Subcategory'),
    ('f2', 'PRODUCT_NAME', 'Product'),
    ('f3', 'CHANNEL', 'Channel'),
]
KINDS = {'REVENUE_SUM': 'money', 'REVENUE_AVG': 'money', 'QUANTITY_SUM': 'number', 'QUANTITY_AVG': 'number', 'GROSS_MARGIN_SUM': 'money', 'GROSS_MARGIN_AVG': 'money', 'NET_REVENUE_SUM': 'money', 'NET_REVENUE_AVG': 'money', 'RETURN_PCT': 'number', 'MARGIN_PCT': 'number', 'NET_PCT': 'number', 'LOST_PCT': 'number', 'AVG_ORDER': 'money', 'DISCOUNT_AVG': 'number', 'RATING_AVG': 'number', 'DELIVERY_DAYS_AVG': 'number', 'BELOW_REORDER_PCT': 'number', 'STOCK_HEADROOM_AVG': 'number', 'CUSTOMERS_N': 'number', 'PRODUCTS_N': 'number'}
MEASURES = ['REVENUE_SUM', 'QUANTITY_SUM', 'GROSS_MARGIN_SUM', 'NET_REVENUE_SUM']
...
ADDITIVE = ['REVENUE_SUM', 'QUANTITY_SUM', 'GROSS_MARGIN_SUM', 'NET_REVENUE_SUM']
KEYS = []
FRAMES = ['trend.sql', 'cut_a.sql', 'cut_b.sql', 'detail.sql']
DETAIL_COLS = ['CATEGORY', 'SUBCATEGORY', 'LINES_N', 'REVENUE_SUM', 'QUANTITY_SUM', 'GROSS_MARGIN_SUM']
MAIN = "REVENUE_SUM"
MAIN_ALT = "REVENUE_AVG"
MAIN_AVG = "REVENUE_AVG"
MAIN_LABEL = "Booked Revenue"
CUT_A, CUT_B = "Category", "Subcategory"
...
NEGATIVES = [
    'No product cost or supplier. Margin is a fixed 45% on every line, so unit economics cannot be compared across products.',
    'No product launch date, version or lifecycle stage.',
    'No stock-out or availability history, so lost demand is invisible.',
    'No review text or return reason — RATING is a restatement of discount, not feedback.',
]
REFUSED = [
    ...
    'Which products are returned most? None distinctly. The return rate is 13.9-14.4% across every category and product, which is inside the noise band at these volumes.',
    'Should any product be repriced for margin? No — margin is a fixed 45% of revenue on every line, so a price change has no differential margin effect to find.',
]
...
ASK_JOINS = [
    ...
    'Only 16 distinct products exist, and exactly 4 of them are live in each of '
    'the four months at one fixed quantity. Per-product revenue is therefore the '
    'generator product assignment, not demand: a product that looks 4x another is '
    'in the quantity-4 month. Never rank products on revenue without saying this.',
    'No product cost, supplier, launch date, version or lifecycle stage, and no '
    'stock-out history, so demand that could not be filled is invisible. '
    'SUBCATEGORY nests inside CATEGORY; PRODUCT_NAME nests inside SUBCATEGORY.',
]
...
def build_insights(kpi, *frames):
    ...
    out.append({
        "tone": "info",
        "text": (f"{_val(rev)} across {int(units):,} units and "
                 f"{int(_f(kpi.get('PRODUCTS_N')))} products"),
        "meaning": (f"{int(lines):,} order lines, average line {_val(kpi.get(MAIN_AVG))} "
                    f"at {_val(rev / units if units else 0)} per unit."),
        "action": "Units and revenue rank differently — check both charts.",
    })
    sp = _spread(cut_a, "AVG_ORDER")
    if sp and sp["lo"]:
        ratio = sp["hi"] / sp["lo"]
        out.append({
            "tone": "warn" if ratio >= 2 else "info",
            "text": (f"{sp['hi_name']} sells a {ratio:.1f}x bigger line than "
                     f"{sp['lo_name']} ({_val(sp['hi'])} against {_val(sp['lo'])})"),
            "meaning": (f"Across {sp['groups']} categories. This is the widest real "
                        f"effect in the dataset, and it is a price-times-quantity "
                        f"effect inside the category, not a customer effect."),
            "action": f"Line value work belongs in {sp['lo_name']}, not in targeting.",
        })
    spu = _spread(cut_a, "QUANTITY_SUM")
    spr = _spread(cut_a, MAIN)
    if spu and spr and spu["hi_name"] != spr["hi_name"]:
        out.append({
            "tone": "warn",
            "text": (f"{spu['hi_name']} moves the most units but "
                     f"{spr['hi_name']} earns the most revenue"),
            "meaning": (f"{int(spu['hi']):,} units against {_val(spr['hi'])}. A units "
                        f"leader that is not a revenue leader is a low-price mix, which "
                        f"a revenue-only ranking hides."),
            "action": f"Do not read {spu['hi_name']} as the strongest category.",
        })
