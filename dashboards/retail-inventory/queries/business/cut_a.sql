-- Breakdown by Product. Uniform NAME/rank shape so one code path serves every cut.
SELECT g."PRODUCT_NAME" AS NAME,
       COUNT(*) AS LINES_N,
  ...
  FROM "RETAIL"."ORDERS_ENRICHED" g
   WHERE ({f0!s} = '*' OR INSTR({f0!s}, '|' || TO_CHAR(g."PRODUCT_NAME") || '|') > 0)
     ...
     AND ({f3!s} = '*' OR INSTR({f3!s}, '|' || TO_CHAR(g."ORDER_STATUS") || '|') > 0) GROUP BY 1 ORDER BY QUANTITY_SUM DESC LIMIT 60
