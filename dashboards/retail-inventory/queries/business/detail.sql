...
SELECT g."PRODUCT_NAME" AS PRODUCT_NAME,
       g."STORE_NAME" AS STORE_NAME,
       COUNT(*) AS LINES_N,
  ...
  FROM "RETAIL"."ORDERS_ENRICHED" g
   WHERE ({f0!s} = '*' OR INSTR({f0!s}, '|' || TO_CHAR(g."PRODUCT_NAME") || '|') > 0)
     AND ({f1!s} = '*' OR INSTR({f1!s}, '|' || TO_CHAR(g."CATEGORY") || '|') > 0)
     AND ({f2!s} = '*' OR INSTR({f2!s}, '|' || TO_CHAR(g."STORE_NAME") || '|') > 0)
     AND ({f3!s} = '*' OR INSTR({f3!s}, '|' || TO_CHAR(g."ORDER_STATUS") || '|') > 0) GROUP BY 1, 2 ORDER BY QUANTITY_SUM DESC LIMIT 200
