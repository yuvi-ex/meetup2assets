...
Project Manager would actually do about each number. Two rules to keep while you do:
...
TITLE = "Retail Order Delivery"
FILTER_SPEC = [
    ('f0', 'ORDER_STATUS', 'Order Status'),
    ('f1', 'STORE_NAME', 'Store'),
    ('f2', 'CHANNEL', 'Channel'),
    ('f3', 'CATEGORY', 'Category'),
]
KINDS = {'REVENUE_SUM': 'money', 'REVENUE_AVG': 'money', 'LOST_REVENUE_SUM': 'money', 'LOST_REVENUE_AVG': 'money', 'NET_REVENUE_SUM': 'money', 'NET_REVENUE_AVG': 'money', 'QUANTITY_SUM': 'number', 'QUANTITY_AVG': 'number', 'RETURN_PCT': 'number', 'MARGIN_PCT': 'number', 'NET_PCT': 'number', 'LOST_PCT': 'number', 'AVG_ORDER': 'money', 'DISCOUNT_AVG': 'number', 'RATING_AVG': 'number', 'DELIVERY_DAYS_AVG': 'number', 'BELOW_REORDER_PCT': 'number', 'STOCK_HEADROOM_AVG': 'number', 'CUSTOMERS_N': 'number', 'PRODUCTS_N': 'number'}
MEASURES = ['REVENUE_SUM', 'LOST_REVENUE_SUM', 'NET_REVENUE_SUM', 'QUANTITY_SUM']
...
ADDITIVE = ['REVENUE_SUM', 'LOST_REVENUE_SUM', 'NET_REVENUE_SUM', 'QUANTITY_SUM']
KEYS = []
FRAMES = ['trend.sql', 'cut_a.sql', 'cut_b.sql', 'detail.sql']
DETAIL_COLS = ['ORDER_STATUS', 'STORE_NAME', 'LINES_N', 'REVENUE_SUM', 'LOST_REVENUE_SUM', 'NET_REVENUE_SUM']
MAIN = "REVENUE_SUM"
MAIN_ALT = "REVENUE_AVG"
MAIN_AVG = "REVENUE_AVG"
MAIN_LABEL = "Booked Revenue"
CUT_A, CUT_B = "Order Status", "Store"
...
NEGATIVES = [
    'No status history. ORDER_STATUS is a single current value, so nothing can be aged and no cycle time between steps can be computed.',
    'No carrier, shipment or tracking data. DELIVERY_DAYS is a duration with no route behind it.',
    'No promised date or SLA, so no line can be called late.',
    'No project, task, owner or milestone anywhere in this data — this board covers order delivery operations, which is the closest thing present.',
]
REFUSED = [
    ...
    'Is any store slow? Not individually. The six stores fall into two tiers - Pune, Chennai and Delhi at 6.0 days, Mumbai, Bengaluru and Hyderabad at 5.0 - with the three inside each tier within 0.02 days of one another. Naming a single slowest store would be reading a rounding difference.'
    'Why do 14% of lines cancel? Unanswerable from this data — there is no reason code and no timestamp on the status change.',
]
...
ASK_JOINS = [
    ...
    'DELIVERY_DAYS falls in exactly two tiers and nothing finer: Hyderabad Hub, '
    'Bengaluru Central and Mumbai West at 5.0 days, Delhi Select, Chennai Marina '
    'and Pune Market at 6.0, with the three stores inside each tier within 0.02 '
    'days of one another. Report the two tiers; naming a single slowest store is '
    'reading a rounding difference.',
    'ORDER_STATUS is a single CURRENT value with no history, so nothing can be '
    'aged and no step-to-step cycle time exists. No carrier, shipment, tracking, '
    'promised date or SLA, so no line can be called late. The 14% cancellation '
    'rate cannot be explained — there is no reason code.',
]
...
def build_insights(kpi, *frames):
    ...
    booked = _f(kpi.get("REVENUE_SUM"))
    net = _f(kpi.get("NET_REVENUE_SUM"))
    lost = _f(kpi.get("LOST_REVENUE_SUM"))
    days = pct("DELIVERY_DAYS_AVG")
    out.append({
        "tone": "info",
        "text": (f"{int(lines):,} order lines, {_val(booked)} booked, "
                 f"{days:.1f} days to deliver on average"),
        "meaning": (f"{_val(net)} reached the customer, {_val(lost)} did not. Delivery "
                    f"time exists only for lines that shipped — Cancelled and Processing "
                    f"lines carry no value, so the average is over completed lines only."),
        "action": "Never divide delivery days by the full line count.",
    })
    statuses = {str(r.get("NAME")): _f(r.get("LINES_N")) for r in cut_a}
    done = statuses.get("Delivered", 0.0)
    if lines and done:
        out.append({
            "tone": "good" if done / lines >= 0.5 else "warn",
            "text": f"{done / lines * 100:.0f}% of lines reach Delivered",
            "meaning": ("The remainder splits across Shipped, Processing, Cancelled and "
                        "Returned. Those four are near-identical in size, which is a "
                        "property of how this data was produced rather than a funnel "
                        "shape worth optimising."),
            "action": "Track the trend of this share, not its level.",
        })
    stuck = statuses.get("Processing", 0.0)
    if stuck and booked:
        stuck_rev = next((_f(r.get(MAIN)) for r in cut_a
                          if str(r.get("NAME")) == "Processing"), 0.0)
        out.append({
            "tone": "warn",
            "text": (f"{int(stuck):,} lines are still Processing, holding "
                     f"{_val(stuck_rev)}"),
            "meaning": (f"{stuck_rev / booked * 100:.1f}% of booked revenue with no "
                        f"delivery date and no outcome. There is no created-at or "
                        f"status-changed timestamp, so these cannot be aged — only counted."),
            "action": "Ask for a status-history feed; ageing is the missing capability.",
        })
    sp = _spread(cut_b, "DELIVERY_DAYS_AVG")
    if sp and sp["mean"]:
        flat = sp["relative"] < FLAT_PCT
        # The stores do not form a ranking, they form two tiers. Reporting a single
        # "slowest store" out of a three-way tie at 6.0 days invents a laggard.
        vals = sorted((_f(r.get("DELIVERY_DAYS_AVG")), str(r.get("NAME")))
                      for r in cut_b if r.get("DELIVERY_DAYS_AVG") is not None)
        slow = [n for v, n in vals if v >= (sp["hi"] - 0.25)]
        fast = [n for v, n in vals if v <= (sp["lo"] + 0.25)]
        tiered = len(slow) > 1 and len(fast) > 1 and len(slow) + len(fast) == len(vals)
        out.append({
            "tone": "good" if flat else "warn",
            "text": (f"Delivery time is {'the same at every store' if flat else 'split into two tiers'} "
                     f"({sp['lo']:.1f}-{sp['hi']:.1f} days)"),
            "meaning": (f"A {sp['relative']:.0f}% spread across {sp['groups']} stores. "
                        + ("No store is a logistics outlier."
                           if flat else
                           (f"{', '.join(slow)} sit at {sp['hi']:.1f} days and "
                            f"{', '.join(fast)} at {sp['lo']:.1f}, with the stores inside "
                            f"each tier within a rounding difference of each other — so "
                            f"this is a two-tier split, not a ranking."
                            if tiered else f"{sp['hi_name']} is slowest."))),
            "action": ("Look at the Processing backlog instead." if flat else
                       "Ask what the slower tier shares — no single store is the problem."
                       if tiered else f"Start with {sp['hi_name']}."),
        })
